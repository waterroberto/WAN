...
const HowItWorks = () => {
  return (
    <section className='p-4 py-16 md:p-16 xl:px-32 bg-white'>
      <p className='mb-8 text-center text-2xl font-extrabold text-gray-800 md:text-3xl'>
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
        {steps.map((step) => (
          <div
            key={step.title}
            className='flex flex-col items-center justify-center gap-4 p-6 text-gray-800 mx-auto max-w-sm'
            data-aos='fade-up'
          >
            <span className='text-primary text-6xl'>{step.icon}</span>
            <p className='text-center font-extrabold text-xl'>{step.title}</p>
            <p className='text-center text-gray-500'>{step.content}</p>
          </div>
        ))}
      </div>
    </section>
